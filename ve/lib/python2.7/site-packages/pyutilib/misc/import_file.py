...
def import_file(filename, context=None, name=None):
    """
    ...
    argument by removing any directory components along with the .py
    (or .pyc) suffix if any of these exist. If the module name already
    appears in sys.modules, the corresponding module entry will be
    returned from this dictionary. Otherwise, an attempt will be made
    import the module name extracted from the filename argument (using
    any directory information that was included).
    ...
    module = None
    # If an import name is not given we will first check if the module
    # name already exists sys.modules due to a previous import and if
    # not we will try to import
    ...
    #           module 'bar' from anywhere else. As a workaround for
    #           this issue, the 'name' keyword was added to this
    #           function, which indicates that the module source
    #           should be obtained loaded from scratch with and
    #           assigned the module name specified by this keyword.
    if name is None:
        name = modulename
        ...
        if is_file:
            ...
            if dirname is not None:
                ...
                try:
                    ...
                    sys.path.remove(implied_dirname)
        try:
            # Note: we are always handing load_source a .py
            #       file, but it will use the .pyc or .pyo 
            #       file if it exists
